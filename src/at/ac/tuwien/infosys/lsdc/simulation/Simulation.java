...
public class Simulation {
	...
	private void execute() {
		...
		try {
			parameters = SimulationParametersFactory.getInstance()
					.createParameters(SIMULATION_PROPERTIES_FILENAME);

			ICloudClusterFactory cloudClusterFactory = LocalCloudClusterFactory
					.getInstance();
			ICloudClusterManager cluster = cloudClusterFactory
					.createLocalCluster(parameters.getPhysicalMachines());
	...
	private static Integer generateNormalDistributedInteger(Integer lowerBound,
			Integer upperBound) {
